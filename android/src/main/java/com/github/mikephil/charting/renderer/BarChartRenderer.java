...
public class BarChartRenderer extends BarLineScatterCandleBubbleRenderer {
    ...
    protected void drawDataSet(Canvas c, IBarDataSet dataSet, int index) {
        boolean drawRoundedBarEnabled = mChart.isDrawRoundedBarEnabled();
        ...
        if (mChart.isDrawBarShadowEnabled()) {
            ...
            for (int i = 0, count = Math.min((int)(Math.ceil((float)(dataSet.getEntryCount()) * phaseX)), dataSet.getEntryCount());
                ...
                if(drawRoundedBarEnabled){
                    c.drawRoundRect(mBarShadowRectBuffer,mBarShadowRectBuffer.width()/2,mBarShadowRectBuffer.width()/2,mShadowPaint);
                }else {
        ...
        for (int j = 0; j < buffer.size(); j += 4) {
            ...
            if(drawRoundedBarEnabled){
                c.drawRoundRect(buffer.buffer[j], buffer.buffer[j + 1], buffer.buffer[j + 2],
                        buffer.buffer[j + 3], (buffer.buffer[j + 2] - buffer.buffer[j])/2,(buffer.buffer[j + 2] - buffer.buffer[j])/2,mRenderPaint);
            }else {
            ...
            if (drawBorder) {
                if(drawRoundedBarEnabled){
                    ...
                    c.drawRoundRect(buffer.buffer[j], buffer.buffer[j + 1], buffer.buffer[j + 2],
                            buffer.buffer[j + 3], (buffer.buffer[j + 2] - buffer.buffer[j])/2,(buffer.buffer[j + 2] - buffer.buffer[j])/2,mBarBorderPaint);
    ...
    public void drawHighlighted(Canvas c, Highlight[] indices) {
        ...
        for (Highlight high : indices) {
            ...
            if(drawRoundedBarEnabled){
                c.drawRoundRect(mBarRect,mBarRect.width()/2,mBarRect.width()/2,mHighlightPaint);
            }else {
